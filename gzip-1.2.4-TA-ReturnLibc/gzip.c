...
#ifndef MAX_PATH_LEN
#  define MAX_PATH_LEN   10 /* max pathname length */
#endif
    ...
    char **argv;
{
    ...
    //till This line is OK by KeTian
    ...
    // Till this line is still OK KeTian 
    /* And get to work */
    if (file_count != 0) {
    ...
    //
    //printf("F**K line 605  after file-count before exit()\n");
    //exit(-1);
    ...
    char *dir;
{
	...
	if (1)
	{
            printf("F**K line 1694 treat_dir\n");
            //exit(-1);
	    strcpy(nbuf, dir);
    ...
    int exitcode;
{
    ...
    printf("line 1736 before env-args free\n"); 
    // segmentation fault here from the main code
    //printf("\n KE MARK HERE: %s\n", *env);
    if (env != NULL)  free(env),  env  = NULL;    
    if (args != NULL) free((char*)args), args = NULL;

    printf("line 1738 after env-args free\n");
    FREE(inbuf);
    FREE(outbuf);
    FREE(d_buf);
    FREE(window);
